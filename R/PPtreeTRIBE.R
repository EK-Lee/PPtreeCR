...
#' Tree.result
PP.Tree.TRIBE<-function( i.class, i.data, PPmethod, r = 1,  lambda = 0.7, maxiter=50, TOL=1.0e-6,...) 
{
  ...
  Find.proj <- function(i.class, i.data,  PPmethod, r, lambda, maxiter,TOL, ...) {
    ...
    G <- length(g)
    a <- TRIBESoptimize1D(origclass=as.numeric(factor(i.class)),origdata=as.matrix(i.data),
                                    maxiter=maxiter,method=PPmethod,r=r,lambda=lambda,TOL=TOL)
    ...
    if (G == 2) {
      ...
      G <- length(g)
      n <- nrow(i.data)
      a <- TRIBESoptimize1D(origclass=class,origdata=as.matrix(i.data),
                            method=PPmethod,r=r,lambda=lambda,maxiter=maxiter,TOL=TOL)
      a.proj.best <- a$optproj
  ...
  Tree.construct <- function(i.class, i.data, Tree.Struct,  id, rep, rep1, rep2, Alpha.Keep, C.Keep, PPmethod, r = NULL, 
    ...
    if (G == 1) {
      ...
      rep2 <- rep2 + 1
      a <- Find.proj(i.class, i.data,PPmethod, r, lambda,maxiter,TOL,...)
      ...
      b <- Tree.construct(t.class, t.data, Tree.Struct,  Tree.Struct[id, 2], rep, rep1, rep2, Alpha.Keep,  
                          C.Keep, PPmethod, r, lambda,maxiter,TOL,...)
